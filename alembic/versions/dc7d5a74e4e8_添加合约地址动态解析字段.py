...
Revision ID: dc7d5a74e4e8
Revises: add_contract_name_simple
Create Date: 2025-09-11 15:33:26.552634
...
revision = 'dc7d5a74e4e8'
down_revision = 'add_contract_name_simple'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.add_column('evm_contract_caller', sa.Column('contract_address_source', sa.String(length=10), nullable=True, comment='合约地址来源：static=固定地址，dynamic=来自JSON字段'))
    op.add_column('evm_contract_caller', sa.Column('contract_address_field', sa.String(length=255), nullable=True, comment='动态合约地址字段名'))
...
def downgrade() -> None:
    ...
    op.alter_column('evm_contract_caller', 'id',
               ...
               existing_nullable=False,
               autoincrement=True)
    op.drop_column('evm_contract_caller', 'contract_address_field')
    op.drop_column('evm_contract_caller', 'contract_address_source')
